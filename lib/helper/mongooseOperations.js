...
async function find(args, options={}) {
  try {
...
async function findOne(args) {
  try {
    const query = args.options.idField
      ? { [args.options.idField]:args.params[args.options.idField] }
      : { _id:args.params._id };
    const data = await args.model.findOne(query);
